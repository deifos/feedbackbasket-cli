...
test('the Commander tree exposes every contract command mapping', () => {
  ...
  for (const operation of PRODUCT_OPERATIONS) {
    for (const command of operation.cli.commands) {
      assert.equal(hasCommand(program, command), true, `${operation.id}: ${command}`);
    }
  }
...
test('all high-impact commands support --yes', () => {
  const program = createProgram();
  for (const operation of PRODUCT_OPERATIONS.filter((item) => item.risk.explicitConfirmation)) {
    for (const commandPath of operation.cli.commands) {
      let current = program;
      for (const token of commandPath.split(' ')) current = findCommand(current, token)!;
      assert.ok(current.options.some((option) => option.long === '--yes'), `${operation.id}: ${commandPath}`);
    }
  }
});

test('the prepublish check rejects version and mapping drift', () => {
  assert.throws(() => verifyCliParity('2.9.9'), /Expected values to be strictly equal/);
  assert.throws(
    () => verifyCliParity('3.0.0', PRODUCT_OPERATIONS.slice(0, -1).map(({ id }) => id)),
