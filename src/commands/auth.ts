...
export function authLoginResult(
    ...
    .action(async (opts) => {
      ...
      try {
        ...
        if (isInteractive) {
          ...
          console.log(
            brand.muted(
              "  Token saved — run feedbackbasket doctor to diagnose",
            ),
          );
          console.log();
        }
      ...
      if (isInteractive) {
        ...
        if (email)
          console.log(
            `    ${brand.muted("Logged in as")} ${brand.bold(email)}`,
          );
        console.log();
      }
      ...
      if (isInteractive && !config.defaultProject) {
        ...
        try {
          ...
          if (projects.length === 0) {
            ...
            console.log(
              `  ${brand.success("✓")} Default project: ${brand.bold(p.name)}`,
            );
            console.log(
              `    ${brand.muted("Only project in your organization")}`,
            );
            console.log();
          } else {
            console.log(
              `  You have ${brand.bold(String(projects.length))} projects:\n`,
            );
            ...
            if (choice !== null) {
              ...
              console.log(
                `    ${brand.muted("Use --project to override per-command")}`,
              );
            } else {
              console.log();
              console.log(
                `  ${brand.muted("Skipped. Use --project or: feedbackbasket config set defaultProject <id>")}`,
              );
            }
            console.log();
          }
          ...
          console.log(
            brand.muted(
              "  You can set a default later: feedbackbasket config set defaultProject <id>",
            ),
          );
          console.log();
        }
      ...
      if (isInteractive) {
        ...
        if (claudeExists && !skillExists) {
          ...
          console.log(
            `    1. Install FeedbackBasket skill to ~/.claude/skills/`,
          );
          console.log();
          ...
          if (doSetup) {
            try {
              ...
              if (existsSync(skillSrc)) {
                ...
                console.log(
                  `  ${brand.error("✗")} SKILL.md not found in package`,
                );
              }
            } catch {
            ...
            console.log(
              brand.muted("  Skipped. Run later: feedbackbasket setup claude"),
            );
          }
          console.log();
      ...
      if (isInteractive) {
        ...
        console.log(
          `  ${brand.success("✓")} Authenticated${email ? ` as ${email}` : ""}`,
        );
        if (defaultProjectName) {
          console.log(
            `  ${brand.success("✓")} Default project: ${defaultProjectName}`,
          );
        } else if (config.defaultProject) {
          console.log(
            `  ${brand.success("✓")} Default project: ${config.defaultProject}`,
          );
        } else {
        ...
        console.log();
        console.log(
          `    ${brand.command("feedbackbasket projects list")}     List your projects`,
        );
        console.log(
          `    ${brand.command("feedbackbasket feedback list")}     View recent feedback`,
        );
        console.log(
          `    ${brand.command("feedbackbasket bugs list")}         View bug reports`,
        );
        console.log(
          `    ${brand.command("feedbackbasket doctor")}            Run diagnostics`,
        );
        console.log();
      ...
      // Machine output
      writer.ok(
        authLoginResult(email, selectedScope, config.defaultProject),
        {
          summary: email
            ? `Logged in as ${email}`
